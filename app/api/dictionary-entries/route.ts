...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    switch (type) {
      case "word":
        entries = await db.select().from(words).orderBy(words.value).limit(10);
        break;
      case "prefix":
        entries = await db
          .select()
          .from(prefixes)
          .orderBy(prefixes.createdAt)
          .limit(10);
        break;
      case "suffix":
        entries = await db
    ...
    return NextResponse.json(
      { error: `Failed to fetch ${type} entries` },
      { status: 500 },
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: `Failed to delete ${type} entry` },
      { status: 500 },
    );
  }
}
